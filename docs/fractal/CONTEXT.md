...
_Avoid_: "Page Editor" (ambiguous with the Page concept itself).
...
_Avoid_: "AI sync", "real-time collaboration" (it's not multi-cursor collaborative editing; it's one-way external-change detection).
...
_Avoid_: "turndown" (implementation detail), "paste converter" (only one of its uses).
...
_Avoid_: "note.json", "manifest" (the file is literally named `outline.note`).
...
### E2E Test (Playwright)
The primary (currently only) test layer. ~90 spec files in `test/specs/` that launch the VS Code extension in a real webview and exercise editor/outliner operations via Playwright. Unit tests are desired but not yet introduced.
