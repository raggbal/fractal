...
Scope: fractal (`.odk/hooks/pre-commit`, `.odk/hooks/commit-msg`)
...
The user explicitly opted out of Conventional Commits enforcement after the second hook failure.

## Decision

Replace `.odk/hooks/pre-commit` and `.odk/hooks/commit-msg` with `exit 0` stubs that include a one-line comment explaining why. The hook files remain in place (so `odk init` does not silently re-enable them on the next run) but become no-ops.

`pre-push` is left at the default for now — it is rarely triggered in this single-developer workflow and can be revisited if it becomes a problem.
...
- No automated lint / typecheck runs at commit time. CI (or `npm run check` / `tsc --noEmit` invoked by hand) is the line of defense.
