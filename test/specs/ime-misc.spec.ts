...
test.describe('IME対応', () => {
    ...
    test('IME入力中のEnter無視（compositionイベント）', async ({ page }) => {
        // IME入力をシミュレート
        await page.evaluate(() => {
    ...
    test('IME入力中のShift+Enter処理', async ({ page }) => {
        await page.evaluate(() => {
            const editor = document.getElementById('editor');
            if (!editor) return;
            editor.dispatchEvent(new CompositionEvent('compositionstart', { data: '' }));
...
test.describe('補助機能', () => {
    ...
    test('アウトライン表示', async ({ page }) => {
        ...
        await page.evaluate(() => {
            ...
            if (typeof (window as any).updateOutline === 'function') {
                (window as any).updateOutline();
            }
        });
        ...
        expect(outlineHtml).toBeDefined();
    });

    test('ワードカウント表示', async ({ page }) => {
