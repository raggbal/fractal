 ...
 * TC-MVR-40 参照リンク [ref](x.md) は move されず URL 書換のみ（subpage ゲート反転の番人）
 *
 * 【subpage marker ゲート反転（ADR-0009）】
 *   `[[label]](x.md)` = サブページ → 再帰複製/移動される
 *   `[label](x.md)`   = 参照リンク → 複製/移動されない（URL 書換のみ）
 *   ※ collectSurvivingMdLinkRefs (TC-MVR-12) は「残留参照判定」なので subpage/参照 両方数える
 *     （mdLinks = 種別問わず全 .md リンク）→ TC-MVR-12 の fixture は参照リンクのまま維持。
 */
...
test('TC-MVR-01 A→B 再帰移動（両方 dst へ、src から消える）', () => {
    ...
    const aId = makeMd(fm, 'A', `see [[b]](${bId}.md)`); // subpage marker → 複製/移動対象
    ...
    // dst の A 本文の [[b]] subpage リンクが dst 内で解決
...
test('TC-MVR-02 多段 A→B→C 全移動', () => {
    ...
    const bId = makeMd(fm, 'B', `[[c]](${cId}.md)`); // subpage
    const aId = makeMd(fm, 'A', `[[b]](${bId}.md)`); // subpage
...
test('TC-MVR-03 循環 A↔B で無限ループせず移動', () => {
    ...
    const aId = makeMd(fm, 'A', `[[b]](${bId}.md)`); // subpage
    fs.writeFileSync(path.join(src, `${bId}.md`), `[[a]](${aId}.md)`, 'utf8'); // subpage 循環
    const newAId = fm.moveFileItemToOtherNote(aId, dst);
    expect(newAId).toBeTruthy();
...
test('TC-MVR-10 残留参照ありの closure md は copy フォールバック（元に残す）', () => {
    ...
    const bId = makeMd(fm, 'B', '# b');
    const aId = makeMd(fm, 'A', `[[b]](${bId}.md)`); // subpage
    const cId = makeMd(fm, 'C', `[[b]](${bId}.md)`); // C も B を subpage 参照、C は src に残る
    fm.moveFileItemToOtherNote(aId, dst);
...
test('TC-MVR-11 残留が移動 closure だけなら move', () => {
    ...
    const fm = new NotesFileManager(src);
    const bId = makeMd(fm, 'B', '# b');
    const aId = makeMd(fm, 'A', `[[b]](${bId}.md)`); // B を subpage 参照するのは A のみ
    fm.moveFileItemToOtherNote(aId, dst);
...
test('TC-MVR-13 move md → copy-fallback md への link が dst 新 id に解決', () => {
    ...
    const bId = makeMd(fm, 'B', '# b body');
    const aId = makeMd(fm, 'A', `[[b]](${bId}.md)`); // subpage
    makeMd(fm, 'C', `[[b]](${bId}.md)`); // C 残留 subpage 参照 → B は copy-fallback
    const newAId = fm.moveFileItemToOtherNote(aId, dst);
...
test('TC-MVR-20 外部note リンクは移動せず dst 相対書換（絶対パス禁止）', () => {
    ...
    const fm = new NotesFileManager(src);
    const bId = makeMd(fm, 'B', '# b');
    // [[b]] = 自note内 subpage（移動対象）、[x] = 自note外リンク（subpage でも解決不能=非複製 / URL 書換のみ）
...
test('TC-MVR-30 部分文字列重複リンクを誤置換しない（B.md / BB.md）', () => {
    ...
    const aId = makeMd(fm, 'A', `[[b]](${bId}.md) [[bb]](${bbId}.md)`); // 両方 subpage
    const newAId = fm.moveFileItemToOtherNote(aId, dst);
    const aBody = fs.readFileSync(path.join(dst, `${newAId}.md`), 'utf8');
...
test('TC-MVR-40 参照リンク [ref](x.md) は move されず src に残り URL 書換のみ（ゲート反転の番人・load-bearing）', () => {
    const { src, dst, cleanup } = setupNotes();
    const fm = new NotesFileManager(src);
    const bId = makeMd(fm, 'B', '# b body');
    // [ref] = 参照リンク（`[[]]` でない）→ 複製/移動されない。URL 書換のみ（ADR-0009 ゲート反転）。
    const aId = makeMd(fm, 'A', `[ref](${bId}.md)`);
    const newAId = fm.moveFileItemToOtherNote(aId, dst);
    expect(newAId).toBeTruthy();
    // A は dst へ移動（src から消える）
    expect(fs.existsSync(path.join(src, `${aId}.md`))).toBe(false);
    // ★load-bearing: 参照リンク先 B は複製も移動もされず src に残る（subpage なら move されていた）
    expect(fs.existsSync(path.join(src, `${bId}.md`))).toBe(true);
    // dst には A のみ（B は複製されない）
    const dstMds = fs.readdirSync(dst).filter(f => f.endsWith('.md'));
    expect(dstMds.length).toBe(1);
    expect(fs.existsSync(path.join(dst, `${bId}.md`))).toBe(false);
    // dst の A 参照リンクは URL 書換され、元 src の B を指し続ける（絶対パス禁止）
    const aBody = fs.readFileSync(path.join(dst, `${newAId}.md`), 'utf8');
    expect(aBody).not.toContain('/Users/');
    const m = aBody.match(/\[ref\]\(([^)]+)\)/);
    expect(m).toBeTruthy();
