...
var notesFilePanel = (function() {
    ...
    // FR-T01: 外部 files（.md のみ）を FileReader で読み、bridge.notesRegisterExternalMd を 1 回呼ぶ。
    // items = 種別付き配列 [{kind:'md', name, content}]（次フェーズのファイル D&D 拡張用に shape を維持）。
    // 非 md は黙ってスキップ・md 0 件なら bridge を呼ばない。挿入位置（parentId/index）は呼び出し側が決定。
    function registerExternalMdFiles(e, parentId, index) {
        var fileList = [];
        if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
            ...
            for (var k = 0; k < items.length; k++) {
                if (items[k].kind === 'file') {
                    ...
                    if (gf) fileList.push(gf);
                }
            }
        }
        // .md のみ抽出（非 md は通知なしでスキップ）
        var mdFiles = fileList.filter(function(f) { return f && /\.md$/i.test(f.name || ''); });
        if (mdFiles.length === 0) return; // md 0 件なら bridge を呼ばない
        var readOne = function(file) {
            return new Promise(function(resolve) {
                var reader = new FileReader();
                reader.onload = function() { resolve({ kind: 'md', name: file.name, content: String(reader.result || '') }); };
                reader.onerror = function() { resolve(null); };
                reader.readAsText(file);
            });
        };
        Promise.all(mdFiles.map(readOne)).then(function(results) {
    ...
    function setupDropTarget(el) {
        el.addEventListener('dragover', function(e) {
            ...
            var fromExternal = !dragItemId && !fromOutliner && !fromOutlinerSubtree && !fromMdSubpage && isExternalFilesDrag(e);
            if (!dragItemId && !fromOutliner && !fromOutlinerSubtree && !fromMdSubpage && !fromExternal) return;
            e.preventDefault();
        ...
        el.addEventListener('drop', function(e) {
            ...
            // 内部 drag / outliner / subpage が全て無いときのみ（内部 drag 最優先を保つ）。
            if (!dragItemId && !outPayload && !subtreePayload && !mdSubpagePayload && isExternalFilesDrag(e)) {
                clearAllDragOver();
                removeDropIndicator();
                ...
                registerExternalMdFiles(e, parentEx, ratioEx < 0.5 ? idxEx : idxEx + 1);
                return;
            }
    ...
    function setupFolderChildrenDrop(childrenEl, folderId) {
        ...
        childrenEl.addEventListener('drop', function(e) {
            ...
            if (!dragItemId && !outPayload && isExternalFilesDrag(e)) {
                ...
                removeDropIndicator();
                lastDropLine = null;
                registerExternalMdFiles(e, folderId, getChildIdsOfParent(folderId).length);
                return;
            }
        ...
        if (listEl && !listEl.__rootDropWired) {
            ...
            listEl.addEventListener('drop', function(e) {
                ...
                if (!dragItemId && !outPayload && !mdSubpagePayloadR && isExternalFilesDrag(e)) {
                    ...
                    registerExternalMdFiles(e, null, rootIdsEx.length);
                    return;
                }
