 ...
 */
// Modified for fractal (ADR-0010): jszip → viewer-common/zip 三重ガード付き façade
import { createZipFacade } from './zip-facade.mjs'
import { readXmlFile } from './readXmlFile.mjs'
import { getBorder } from './border.mjs'
import { getSlideBackgroundFill, getShapeFill, getSolidFill, getPicFill, getPicFilters, getImageData, getVideoData, getAudioData } from './fill.mjs'
import { getChartInfo } from './chart.mjs'
import { getVerticalAlign, getTextAutoFit } from './paragraph.mjs'
import { getTextInsets } from './textInsets.mjs'
import { getPosition, getSize } from './position.mjs'
...
import { findOMath, latexFormart, parseOMath } from './math.mjs'
import { getShapePath } from './shapePath.mjs'
import { parseTransition, findTransitionNode } from './animation.mjs'
import { getDiagramNodeContext, getSmartArtTextData } from './diagram.mjs'
...
export async function parse(file, options = {}) {
  ...
  const zip = await createZipFacade(file)
...
async function genShape(node, slideLayoutSpNode, slideMasterSpNode, name, type, order, warpObj, source, link, groupHierarchy = []) {
  ...
  // Modified for fractal (FR-PPV-03): raw vert を露出（eaVert 以外の vert/vert270 をレンダラで解釈）
  const vertRaw = getTextByPathList(node, ['p:txBody', 'a:bodyPr', 'attrs', 'vert']) || ''
  const isVertical = vertRaw === 'eaVert'
  ...
  return {
    ...
    type: 'text',
    isVertical,
    vert: vertRaw, // Modified for fractal (FR-PPV-03)
    rotate: txtRotate,
  }
}
...
async function processPicNode(node, warpObj, source) {
  ...
  if (videoNode) {
    ...
    if (isVideoLink(videoFile)) {
      // Modified for fractal: escapeHtml 廃止（リンク URL は raw のまま — レンダラは CSSOM/attribute 設定のみ）
      isVdeoLink = true
    } 
    else {
