...
*/
    ...
    function setupSearchBar() {
        ...
        // TASK-B7: Switch to Table view button — inserted to the left of the
        // search input wrapper. Layout constraint: search wrapper is to the
        // right of the search-mode-toggle and to the left of undo/redo/menu;
        // the Switch button slots between toggle and the input wrapper so it
        // is visually adjacent to the search input but does not overlap with
        // the pinned tag bar (which lives in a separate row below).
        try {
            var bar = searchInput.closest ? searchInput.closest('.outliner-search-bar') : null;
            var wrapper = searchInput.closest ? searchInput.closest('.outliner-search-input-wrapper') : null;
            if (bar && wrapper && !bar.querySelector('.outliner-switch-view-btn')) {
                var switchBtn = document.createElement('button');
                switchBtn.type = 'button';
                switchBtn.className = 'outliner-switch-view-btn';
                var label = (i18n.outlinerSwitchToTable || 'Switch to Table view');
                switchBtn.title = label;
                switchBtn.setAttribute('aria-label', label);
                switchBtn.innerHTML =
                    '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"' +
                    ' fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"' +
                    ' stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>' +
                ...
                switchBtn.addEventListener('click', function (e) {
                    ...
                    if (host && typeof host.requestReopenAs === 'function') {
                        host.requestReopenAs('fractal.outlinerTable');
                    }
                });
                bar.insertBefore(switchBtn, wrapper);
            }
        } catch (_) { /* ignore — non-critical UI */ }
