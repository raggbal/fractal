...
class EditorInstance {
    ...
    // Add resize handles to header AND body cells (any cell's right edge can resize the column).
    //
    // BUG-FIX: handle は cell の **先頭** に挿入する。末尾に置くと:
    //   1. ArrowRight で cell 末尾に到達した cursor が handle の前/後に挟まれて右端に居着いてしまう
    //   2. lastChild 判定する shift+enter ロジック等が壊れる
    //   先頭に置いても position:absolute なので見た目は変わらず、cursor は通常通り content 末尾で
    //   次の cell へ抜ける。
    ...
    function handleResizeStart(e) {
        ...
        resizingCell = handle.closest('th, td');
        resizingTable = handle.closest('table');
        resizingColIndex = parseInt(handle.dataset.colIndex || '0', 10);
        ...
        // Highlight ALL handles in the same column (visual feedback for whole column)
        if (resizingTable) {
            resizingTable.querySelectorAll('.table-col-resize-handle[data-col-index="' + resizingColIndex + '"]').forEach((h) => {
    ...
    function handleResizeEnd(e) {
        ...
        // Remove resizing class from ALL handles in the column
        if (resizingTable) {
            resizingTable.querySelectorAll('.table-col-resize-handle.resizing').forEach((h) => {
                h.classList.remove('resizing');
            });
            // Persistence (option C): record final widths to data-col-widths attribute,
            // then sync — htmlToMarkdown emits `<!-- fractal-col-widths: ... -->` before the table.
            try {
                ...
                // Trigger sync so the comment line is written to the doc
    ...
    // Whole-column hover highlight via event delegation.
    // mouseover: when entering a handle, mark all handles in same column with .col-hover.
    // mouseout: clear .col-hover unless moving to a sibling handle in the same column.
    editor.addEventListener('mouseover', function(e) {
        const handle = e.target.closest && e.target.closest('.table-col-resize-handle');
        if (!handle) return;
        const table = handle.closest('table');
        if (!table) return;
        const colIndex = handle.dataset.colIndex;
        // clear existing col-hover (on any column)
        table.querySelectorAll('.table-col-resize-handle.col-hover').forEach(h => h.classList.remove('col-hover'));
        table.querySelectorAll('.table-col-resize-handle[data-col-index="' + colIndex + '"]')
            .forEach(h => h.classList.add('col-hover'));
    });
    editor.addEventListener('mouseout', function(e) {
        const handle = e.target.closest && e.target.closest('.table-col-resize-handle');
        if (!handle) return;
        const related = e.relatedTarget && e.relatedTarget.closest && e.relatedTarget.closest('.table-col-resize-handle');
        if (related && related.dataset.colIndex === handle.dataset.colIndex) return;
        const table = handle.closest('table');
        if (!table) return;
        table.querySelectorAll('.table-col-resize-handle.col-hover').forEach(h => h.classList.remove('col-hover'));
    });

    // ArrowLeft/Right で cell 先頭/末尾に到達した cursor が handle (contenteditable=false)
    // の前後に「居着く」のを防ぐ。
    // 先頭に handle を入れたので、ArrowLeft 時の `TD offset = 1` (between handle and content) と
    // `TD offset = 0` (before handle) の 2 段階を 1 押しでスキップして、前 cell の末尾へ行く。
    editor.addEventListener('keydown', function(e) {
        ...
        const cell = sc.nodeType === Node.ELEMENT_NODE
            ? (sc.tagName === 'TD' || sc.tagName === 'TH' ? sc : null)
            : null;
        if (!cell) return;
        const handle = cell.querySelector(':scope > .table-col-resize-handle');
        if (!handle) return;
        const handleIdx = Array.from(cell.childNodes).indexOf(handle);
        const off = range.startOffset;
        // 先頭 handle (handleIdx === 0):
        //   ArrowLeft at offset 0 (before handle) または offset 1 (between handle and content)
        //     → 前 cell の末尾へ
        //   ArrowRight at offset 0 (before handle) → offset 1 をスキップして content の先頭へ
        if (handleIdx === 0) {
            if (e.key === 'ArrowLeft' && off <= 1) {
                e.preventDefault();
                const prevCell = cell.previousElementSibling
                    || (cell.parentElement && cell.parentElement.previousElementSibling
                        && cell.parentElement.previousElementSibling.lastElementChild);
                if (prevCell) {
                    const r = document.createRange();
                    r.selectNodeContents(prevCell);
                    r.collapse(false); // end of prev cell
                    sel.removeAllRanges();
                    sel.addRange(r);
                }
                return;
            }
            if (e.key === 'ArrowRight' && off === 0) {
                // skip past handle to content start
                e.preventDefault();
                const r = document.createRange();
    ...
    // BUG-FIX: Enter/Add Row 等で新規 <tr>/<td> が挿入されても resize handle が付かない問題。
    // MutationObserver で動的追加を監視 → 親 table に handles を再適用 (idempotent)。
    try {
        const _tableMutationObserver = new MutationObserver(function(mutations) {
            const tablesNeedingHandles = new Set();
            for (const mut of mutations) {
                ...
                for (const node of mut.addedNodes) {
                    ...
                    if (tag === 'TABLE') {
                        tablesNeedingHandles.add(node);
                    } else if (tag === 'TR' || tag === 'TD' || tag === 'TH') {
                        const t = node.closest && node.closest('table');
                        if (t) tablesNeedingHandles.add(t);
                    } else if (node.querySelectorAll) {
                        node.querySelectorAll('table').forEach(t => tablesNeedingHandles.add(t));
                    }
                }
            }
            tablesNeedingHandles.forEach((t) => {
                if (editor.contains(t)) addTableResizeHandles(t);
            });
        });
        _tableMutationObserver.observe(editor, { childList: true, subtree: true });
    } catch (e) { /* MutationObserver unavailable in some envs — fall through */ }
