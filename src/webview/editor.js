...
class SidePanelHostBridge {
    ...
    attachTreeFileToMd(id) {
        if (this._onImageRequest) this._onImageRequest();
        if (typeof this._mainHost.attachTreeFileToMd === 'function') {
            this._mainHost.attachTreeFileToMd(id, this.filePath);
        }
    }
    // FR-TF-19 (§4m 2026-08-10): outliner 📎/page node・md 📎/subpage リンク → sidepanel md D&D の
    // 委譲 4 メソッド。宛先 = this.filePath（sidepanel の md）。cross-note 解決は host（FR-TF-18）。
    attachOutNodeFileToMd(payload) {
        if (this._onImageRequest) this._onImageRequest();
        if (typeof this._mainHost.attachOutNodeFileToMd === 'function') {
            this._mainHost.attachOutNodeFileToMd(payload, this.filePath);
        }
    }
    importOutPageNodeToMd(payload) {
        if (this._onImageRequest) this._onImageRequest();
        if (typeof this._mainHost.importOutPageNodeToMd === 'function') {
            this._mainHost.importOutPageNodeToMd(payload, this.filePath);
        }
    }
    attachMdFileLinkToMd(payload) {
        if (this._onImageRequest) this._onImageRequest();
        if (typeof this._mainHost.attachMdFileLinkToMd === 'function') {
            this._mainHost.attachMdFileLinkToMd(payload, this.filePath);
        }
    }
    linkMdSubpageToMd(payload) {
        if (this._onImageRequest) this._onImageRequest();
        if (typeof this._mainHost.linkMdSubpageToMd === 'function') {
            this._mainHost.linkMdSubpageToMd(payload, this.filePath);
        }
    }
    // MD-45/46/47: drawio
    ...
    requestCreateDrawio() {
        ...
        // insertImageHtml response が main editor に流れて side panel に挿入されない。
        if (this._onImageRequest) this._onImageRequest();
    ...
    // outliner node paste の添付複製 (sprint 20260727-124904): 宛先 = sidepanel の md (this.filePath)。
    // SidePanelHostBridge は共有 factory 非経由の手書きクラスなので個別追加が必要
    // (designer_failures 2026-07-20)。
    pasteOutlinerNodesWithAssets(plainText, nodes) {
        if (typeof this._mainHost.pasteOutlinerNodesWithAssets === 'function') {
            this._mainHost.pasteOutlinerNodesWithAssets(plainText, nodes, this.filePath);
        }
    }
...
class EditorInstance {
    ...
    function markdownToHtmlFragment(markdownText) {
        ...
        function renderTable(rows) {
            ...
            rows.forEach((row, idx) => {
                ...
                cells.forEach((cell, colIdx) => {
                    ...
                    // Compose style: text-align (td only) + optional width (when persisted)
                    let styleParts = [];
                    if (!isHeader) styleParts.push('text-align: ' + align);
                    if (pendingTableColWidths && pendingTableColWidths[colIdx]) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // before a table sets explicit column widths. Skip emitting the comment as content;
            // remember widths and apply to the next table.
            const colWidthsMatch = line.match(/^<!--\s*fractal-col-widths:\s*([0-9,\s]+)\s*-->\s*$/);
            if (colWidthsMatch) {
                const ws = colWidthsMatch[1].split(',')
    ...
    // Resize-handle DOM approach was abandoned (contenteditable=false elements inside cells
    // trap the cursor and break ArrowRight/Shift+Enter behavior). Now resize is detected via
    // mouse position relative to cell's right edge, with visual indicator via CSS ::after
    // pseudo-element. No DOM elements added to cells.
    //
    // Backward compat: if existing tables have stale `.table-col-resize-handle` divs from a
    // previous render, remove them.
    function addTableResizeHandles(table) {
        if (!table) return;
        const stale = table.querySelectorAll('.table-col-resize-handle');
        stale.forEach((h) => { try { h.remove(); } catch (e) { /* ignore */ } });
    }
    ...
    //
    // BUG-FIX (右列 resize で他列まで縮む):
    //   旧実装は totalWidth を th.offsetWidth から計算していた。table-layout: fixed では
    //   sum(col widths) > table.width の時、ブラウザが他列を**縮めて** fit させる。
    //   その縮んだ offsetWidth を sum すると table.width が誤って小さくなり、他列の
    //   style.width が事実上無視される連鎖が発生する。
    //
    //   修正: 全列の **explicit style.width** を array で管理 → 変更列のみ差し替え →
    //   table.width = sum(array) を **先に** 反映してから各 cell の style.width を再設定。
    //   これで他列は元の値のまま保たれる。
    function updateColumnWidth(table, colIndex, newWidth) {
        ...
        // 各列の explicit width を集める。設定されていない列は offsetWidth から拾う (初期化用)。
        const widths = [];
        headerCells.forEach((c, i) => {
            ...
            const sw = c.style.width || '';
            const m = sw.match(/^(\d+(?:\.\d+)?)px$/);
            widths.push(m ? Math.round(parseFloat(m[1])) : Math.max(c.offsetWidth, minWidth));
        });

        const totalWidth = widths.reduce((s, w) => s + w, 0);

        // table.width を **先に** 設定 (sum >= cells の合計 → ブラウザが他列を縮めない)
        table.style.width = totalWidth + 'px';

        // 全 row に同じ widths 配列を適用
        const allRows = table.querySelectorAll('tr');
        allRows.forEach((row) => {
            const cells = row.querySelectorAll('th, td');
            widths.forEach((w, i) => {
                if (cells[i]) cells[i].style.width = w + 'px';
            });
        });
    }
    
    // Resize start — invoked when mousedown fires near a cell's right edge.
    let resizingColIndex = -1;
    let resizingPendingFrame = 0;
    let resizingPendingClientX = 0;
    function handleResizeStart(e, cell) {
        if (!cell) return;
        e.preventDefault();
        e.stopPropagation();
        ...
        resizingColIndex = cell.cellIndex;
        ...
        // Highlight ALL cells in the same column with .col-resize-resizing
        if (resizingTable) {
            // Clear any hover state first
            resizingTable.querySelectorAll('.col-resize-hover').forEach((c) => c.classList.remove('col-resize-hover'));
            resizingTable.querySelectorAll('tr').forEach((row) => {
                const c = row.cells[resizingColIndex];
                if (c) c.classList.add('col-resize-resizing');
            });
        }
    ...
    // Handle resize drag — use absolute mouse position relative to cell's left edge
    // This eliminates mouse↔line desync that occurs with delta-based math when min-width
    // clamping or content reflow shifts the rendered cell width.
    function handleResizeMove(e) {
        if (!isTableColResizing || !resizingCell || !resizingTable) return;
        e.preventDefault();
        // Throttle to one update per animation frame to avoid jank from rapid mousemove
        resizingPendingClientX = e.clientX;
        if (resizingPendingFrame) return;
        resizingPendingFrame = requestAnimationFrame(() => {
            resizingPendingFrame = 0;
            if (!isTableColResizing || !resizingCell || !resizingTable) return;
            // Compute newWidth = mouse X − cell's left edge (absolute, viewport-relative)
            // → cell's right edge always aligns with mouse cursor (clamped to min-width).
            const cellRect = resizingCell.getBoundingClientRect();
            const newWidth = resizingPendingClientX - cellRect.left;
            updateColumnWidth(resizingTable, resizingColIndex, newWidth);
        });
    }
    ...
    function handleResizeEnd(e) {
        ...
        if (resizingPendingFrame) {
            cancelAnimationFrame(resizingPendingFrame);
            resizingPendingFrame = 0;
        }

        // Remove resizing class from ALL cells in the column
        if (resizingTable) {
            resizingTable.querySelectorAll('.col-resize-resizing').forEach((c) => {
                c.classList.remove('col-resize-resizing');
            });
            // Persistence (option C)
            try {
                const headerCells = resizingTable.querySelectorAll('tr:first-child > th, tr:first-child > td');
                const widths = [];
                headerCells.forEach((c) => widths.push(Math.round(c.offsetWidth)));
                if (widths.length > 0) {
                    resizingTable.setAttribute('data-col-widths', widths.join(','));
                }
    ...
    // ===== Cell-edge resize detection (no DOM handles, pure mouse-position based) =====
    //
    // Detect mouse near right edge (within EDGE_PX) of any th/td → set hover class on all
    // cells in the same column. mousedown in that zone starts resize. The pseudo-element
    // ::after on cells with .col-resize-hover / .col-resize-resizing draws the visual blue bar.
    const EDGE_PX = 6;

    function findCellEdgeAt(target, clientX) {
        if (!target || !target.closest) return null;
        const cell = target.closest('th, td');
        if (!cell || !editor.contains(cell)) return null;
        const rect = cell.getBoundingClientRect();
        const distFromRight = rect.right - clientX;
        if (distFromRight >= -2 && distFromRight <= EDGE_PX) return cell;
        return null;
    }

    function clearColHover(table) {
        if (!table) return;
        table.querySelectorAll('.col-resize-hover').forEach((c) => c.classList.remove('col-resize-hover'));
    }

    editor.addEventListener('mousemove', function(e) {
        if (isTableColResizing) return; // resize in progress, don't fiddle with hover
        const cell = findCellEdgeAt(e.target, e.clientX);
        if (!cell) {
            // not on a cell edge → clear all hover in any table within editor
            editor.querySelectorAll('table').forEach((t) => clearColHover(t));
            return;
        }
        const table = cell.closest('table');
        if (!table) return;
        ...
        table.querySelectorAll('tr').forEach((row) => {
            ...
            if (c) c.classList.add('col-resize-hover');
        });
    });
    ...
    editor.addEventListener('mousedown', function(e) {
        const cell = findCellEdgeAt(e.target, e.clientX);
        if (!cell) return;
        handleResizeStart(e, cell);
    });

    // Initialize (cleanup any stale handles from older renders)
    ...
    /**
     * 行頭マーカー（"- " "1. " "# " "> " "[x] " 等）ぶんのテキストだけを削り、残りの childNodes
     * （anchor / subpage リンク / 画像 / 装飾等の inline 要素を含む）を clone した fragment を返す。
     * 変換系が textContent で行を再構築すると inline 要素が全て脱落するバグ
     * （sprint 20260728-121645）の共通修正。markerLen はマーカーの文字数
     * （= textContent 長 − マーカー後残テキスト長）。
     * opts.skipNestedLists: li 内変換用（nested ul/ol は collect/restore 側が別途温存）
     * opts.skipCheckbox: li 内変換用（先頭 checkbox を持ち込まない）
     */
    function extractContentAfterMarker(node, markerLen, opts) {
        opts = opts || {};
        ...
        for (var i = 0; i < node.childNodes.length; i++) {
            ...
            if (child.nodeType === 1) {
                ...
                if (opts.skipNestedLists && (tag === 'ul' || tag === 'ol')) continue;
                if (opts.skipCheckbox && child.tagName === 'INPUT') continue;
                if (tag === 'br') continue; // 空行プレースホルダは持ち込まない
            }
            if (remaining > 0) {
                var t = child.textContent || '';
                if (child.nodeType === 3) {
                    if (t.length <= remaining) { remaining -= t.length; continue; }
                    frag.appendChild(document.createTextNode(t.slice(remaining)));
                    remaining = 0;
                    continue;
                }
                // マーカーが要素内で終わることは通常ない（マーカーは行頭の生テキスト）が、
                // 要素の text 長ぶんは消費して安全側に倒す
                if (t.length <= remaining) { remaining -= t.length; continue; }
    ...
    /** extractContentAfterMarker の結果を el に流し込む（空なら <br>）。 */
    function fillWithExtracted(el, frag) {
    ...
    /** toolbar 変換用: node の inline 内容（link/subpage 等ごと）を clone した fragment を返す。
     *  node が UL/OL のときは従来どおり textContent に縮退（リスト全体の 1 行化は既存挙動を維持）。 */
    function cloneInlineFromNode(node) {
        ...
        return extractContentAfterMarker(node, 0);
    }
    ...
    function checkBlockPatterns(trigger) {
        ...
        if (liNode && liNode.tagName && liNode.tagName.toUpperCase() === 'LI' && trigger === 'space') {
            ...
            if (taskInListMatch) {
                ...
                const inlineFrag = existingText
                    ? extractContentAfterMarker(liNode, liDirectText.length - existingText.length,
                        { skipNestedLists: true, skipCheckbox: true })
                    : null;
                ...
                let textNode = null;
                if (inlineFrag) {
                    textNode = inlineFrag.firstChild && inlineFrag.firstChild.nodeType === 3 ? inlineFrag.firstChild : null;
                    liNode.appendChild(inlineFrag);
                }
        ...
        if (headingMatch && trigger === 'space') {
            ...
            if (existingText) {
                // inline 要素（link/subpage 等）を保持したままマーカーだけ剥がす（sprint 20260728-121645）
                fillWithExtracted(heading, extractContentAfterMarker(node, text.length - existingText.length));
            } else {
        ...
        if (taskMatch && trigger === 'space') {
            ...
            if (existingText) {
                ...
                li.appendChild(extractContentAfterMarker(node, text.length - existingText.length));
            }
        ...
        if (ulMatch && trigger === 'space') {
            ...
            const li = document.createElement('li');
            if (existingText) {
                // inline 要素（link/subpage 等）を保持（sprint 20260728-121645 バグ修正）
                fillWithExtracted(li, extractContentAfterMarker(node, text.length - existingText.length));
            } else {
                li.innerHTML = '<br>';
            }
        ...
        if (bqMatch && trigger === 'space') {
            ...
            if (existingText) {
                // inline 要素保持（sprint 20260728-121645）
                fillWithExtracted(blockquote, extractContentAfterMarker(node, text.length - existingText.length));
            } else {
    ...
    function mdProcessNode(node, listPrefix = '') {
        ...
        switch (tag) {
            ...
            case 'div': 
                ...
                return divContent + '\n';
            // sprint 20260806-035923 (FR-LC-03): bare <br>（li ラッパ無しの部分選択コピー等）は
            // 改行として出す（従来 '' で継続行が連結されていた。段落末尾の装飾 <br> は
            // mdProcessNode に単体で来ることが稀なため、改行出力が安全側）
            case 'br': return '\n';
            case 'hr': return '---\n';
            case 'blockquote':
    ...
    function mdProcessTable(table) {
        ...
        let md = '';
        // Persistence (option C): emit <!-- fractal-col-widths: w1,w2,... --> before the table
        // so column widths survive disk roundtrip. Reader-side detects the comment and applies widths.
        const colWidthsAttr = table.getAttribute('data-col-widths');
    ...
    editor.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Backspace') {
            ...
            if (liElement) {
                ...
                // sprint 20260730-071730 (FR-OLB-01): ol li の行頭 backspace 1 回目はバレット格下げ
        ...
        if (e.key === 'Enter') {
            ...
            if (e.shiftKey && listItem && !preElement && !tableCell) {
                ...
                // <br> の後に実コンテンツ（テキスト/要素。ネストリスト以外）が無い場合のみ、
                // trailing <br> 仕様（視覚行が出ない）対策の 2 個目 <br> を置く。
                // 文字途中の Shift+Enter で無条件に 2 個置くと空白行が挟まる（bug 2026-08-06）。
                let lcHasAfter = false;
                for (let lcN = lcBr.nextSibling; lcN; lcN = lcN.nextSibling) {
                    if (lcN.nodeType === 3 && lcN.textContent !== '') { lcHasAfter = true; break; }
                    if (lcN.nodeType === 1 && lcN.tagName !== 'UL' && lcN.tagName !== 'OL' && lcN.tagName !== 'BR') { lcHasAfter = true; break; }
                }
                if (!lcHasAfter) {
                    lcBr.after(document.createElement('br'));
                }
            ...
            if (listItem && editor.contains(listItem)) {
                ...
                if (e.altKey && !e.shiftKey && !e.metaKey && !e.ctrlKey) {
                    e.preventDefault();
                    // sprint 20260730-071730 (FR-LOE-01): カーソルが行途中なら、カーソル以降の
                    // テキスト（inline 要素を fragment ごと = link/色 span/img 保持）を子 LI へ折り返す。
                    // 通常 Enter split (:8815 付近) と同方式: nested ul/ol の手前までを range 化して extract。
                    // カーソルが行末なら afterFrag が空 = 従来どおり空の子（FR-LOE-02 挙動不変）。
                    let afterFrag = null;
                    let hasAfter = false;
                    if (sel.isCollapsed && sel.rangeCount) {
                        ...
                        try {
                            ...
                            // img/checkbox は textContent が空でも real content（editor.js:8662 の規約）
                            hasAfter = afterFrag.textContent.trim() !== '' ||
                                afterFrag.querySelector('img, input') !== null;
                        } catch (ex) {
                            afterFrag = null;
                            hasAfter = false;
                        }
                        // 折り返し後、LI テキスト部の末尾に残った空 text/br を掃除（nested list は温存）
                        if (hasAfter) {
                            for (let i = listItem.childNodes.length - 1; i >= 0; i--) {
                                const child = listItem.childNodes[i];
                                if (child.nodeType === 1 && (child.tagName === 'UL' || child.tagName === 'OL')) {
                    ...
                    // 新規 child LI を作成 (折り返しあり=fragment / なし=空、checkbox あれば継承)
    ...
    // sprint 20260730-071730 (FR-OLB-01): ol li の行頭 backspace 1 回目のバレット格下げ。
    ...
    function demoteOlLiToBullet(liElement, sel) {
        ...
        var r = document.createRange();
        var target = liElement.firstChild;
        if (target && target.nodeType === 3) {
            r.setStart(target, 0);
        } else if (target) {
            r.setStartBefore(target);
        } else {
            r.setStart(liElement, 0);
        }
        r.collapse(true);
        sel.removeAllRanges();
        sel.addRange(r);
    ...
    function changeParentListType(li, targetTag, olStart) {
        ...
        if (siblings.length === 1) {
            ...
            for (var i = 0; i < parentList.attributes.length; i++) {
                ...
                // sprint 20260730-071730 (ADRL-LST-2): ol の start を ul/非 ol に持ち込まない（stale 属性防止）
            ...
            if (afterItems.length > 0) {
                ...
                // sprint 20260730-071730 (ADRL-LST-2): ol を分割したとき、after 側の表示番号を
                // 変えない（開始番号を無言で変えない = ADRL-0023 の思想）。after の先頭項目の
        ...
        switch (action) {
            ...
            case 'drawio':
                ...
                if (typeof host.requestCreateDrawio === 'function') {
                    // BUG-FIX (cmd+/ で drawio が挿入されないケース):
                    // host 往復中に focus/selection が失われると insertImageHtml で挿入位置が不定になる。
                    // 不可視 marker span を cursor 位置に置いて固定化、insertImageHtml 受信時に marker を
                    // image に置換する。selection が editor 外なら editor 末尾に新規 <p> として pin。
                    try {
                        ...
                        var dwInsertedInEditor = false;
                        if (dwSel && dwSel.rangeCount) {
                            var dwRange = dwSel.getRangeAt(0);
                            if (editor.contains(dwRange.startContainer) || dwRange.startContainer === editor) {
                                ...
                                dwInsertedInEditor = true;
                            }
                        }
                        if (!dwInsertedInEditor) {
                            // Selection editor 外 → 末尾に新規 <p> + marker (常に visible)
                            var dwTailP = document.createElement('p');
                            dwTailP.appendChild(marker);
                            editor.appendChild(dwTailP);
                            try {
                                editor.focus({ preventScroll: false });
                                var dwNewRange = document.createRange();
                                dwNewRange.setStartAfter(marker);
                                dwNewRange.setEndAfter(marker);
                                var dwSel2 = window.getSelection();
                                dwSel2.removeAllRanges();
                                dwSel2.addRange(dwNewRange);
                            } catch (eFoc) { /* ignore */ }
                        }
                    } catch (e) { /* ignore — fallback to focus-based insertion */ }
                    // BUG-FIX (drawio insertion 失敗対策): marker が DOM 操作 (updateFromMarkdown
                    // diff/patch、外部ファイル変更 echo 等) で消失しても fallback できるよう、
                    // cursor 位置を JS 変数にも別途保存しておく。
                    try {
                        window.__pendingDrawioCursor = null;
                        var dwSel3 = window.getSelection();
                        if (dwSel3 && dwSel3.rangeCount) {
                            var r = dwSel3.getRangeAt(0);
                            if (editor.contains(r.startContainer) || r.startContainer === editor) {
                                window.__pendingDrawioCursor = {
    ...
    function showAttachmentsPanel(anchorBtn) {
        ...
        // Resolve to absolute fs path (for Copy Path):
        //   1) rel が既に絶対 (file://, /...) ならそのまま
        //   2) resolveImagePath(rel) で documentBaseUri と join → webview URL
        //   3) cleanImageSrc で webview prefix を除去 → 絶対 fs path
        function relToAbsFs(rel) {
            ...
            try {
                var resolved = (typeof resolveImagePath === 'function')
                    ? resolveImagePath(clean)
                    : '';
                if (typeof cleanImageSrc === 'function') {
                    var stripped = cleanImageSrc(resolved);
                    if (stripped && stripped.startsWith('/')) return stripped;
                }
                // fallback: documentBaseUri を直接処理 (webview URL or file://)
                if (typeof documentBaseUri === 'string' && documentBaseUri) {
                    var base = documentBaseUri.replace(/\/$/, '');
                    // strip webview prefix
                    base = base.replace(/^https:\/\/file(?:\+|%2B)\.vscode-resource\.vscode-cdn\.net/, '');
                    // strip file:// prefix
                    base = base.replace(/^file:\/\//, '');
                    var sub = clean.replace(/^\.\//, '');
                    return base + '/' + sub;
                }
            } catch (e) { /* fallthrough */ }
            return clean;
    ...
    function convertToHeading(level) {
        ...
        const heading = document.createElement('h' + level);
        // inline 要素（link/subpage 等）保持（sprint 20260728-121645）
        fillWithExtracted(heading, cloneInlineFromNode(node));
    ...
    function convertToParagraph() {
        ...
        const p = document.createElement('p');
        // inline 要素保持（sprint 20260728-121645）
        fillWithExtracted(p, cloneInlineFromNode(node));
        node.replaceWith(p);
        setCursorToEnd(p);
        syncMarkdown();
    }
    ...
    function convertToList(type) {
        ...
        // sprint 20260729-000358 (TASK-04): 新規項目は start を持たない = 実効 start 1・li 数 1 の
        // 暫定リストとして areListsCompatible の連続性判定を通す。
        // prepend（新項目が先頭になる）: next に明示 start があると getEffectiveOlStart(next) !== 2 で
        // 非互換 → 独立 ol を作り、隣接 ol の開始番号を無言でシフトさせない（NFR-OLS-01/03）。
        // append（prev の連番を継ぐ）: 新項目側に start は無いので従来どおり常に merge 可。
        const canMergeAsFirstItem = (nextList) => {
            if (type !== 'ol') return true;
            const probe = document.createElement('ol');
            probe.appendChild(document.createElement('li'));
            return areListsCompatible(probe, nextList);
        };

        // Check if the next sibling is the same type of list
        if (nextSibling && nextSibling.tagName.toLowerCase() === type && canMergeAsFirstItem(nextSibling)) {
        ...
        // （append は新項目が prev の連番を継ぐだけで start 破壊が起きないため連続性チェック不要 =
        //   areListsCompatible(prev, 暫定ol) は b 側 start 無しで常に true。raw 判定と等価）
        const prevSibling = node.previousElementSibling;
        if (prevSibling && prevSibling.tagName.toLowerCase() === type) {
            // Merge with existing list - append new item
            const li = document.createElement('li');
            fillWithExtracted(li, cloneInlineFromNode(node)); // inline 保持
            prevSibling.appendChild(li);
            node.remove();
            setCursorToEnd(li);
            syncMarkdown();
    ...
    function convertToTaskList() {
        ...
        if (nextSibling && nextSibling.tagName.toLowerCase() === 'ul') {
            ...
            if (firstLi && firstLi.querySelector('input[type="checkbox"]')) {
                ...
                checkbox.type = 'checkbox';
                li.appendChild(checkbox);
                li.appendChild(cloneInlineFromNode(node)); // inline 保持（sprint 20260728-121645）
                nextSibling.insertBefore(li, nextSibling.firstChild);
                node.remove();
                setCursorToEnd(li);
                syncMarkdown();
        ...
        if (prevSibling && prevSibling.tagName.toLowerCase() === 'ul') {
            ...
            if (firstLi && firstLi.querySelector('input[type="checkbox"]')) {
                ...
                checkbox.type = 'checkbox';
                li.appendChild(checkbox);
                li.appendChild(cloneInlineFromNode(node)); // inline 保持
                prevSibling.appendChild(li);
                node.remove();
                setCursorToEnd(li);
                syncMarkdown();
    ...
    host.onMessage(function(message) {
        ...
        if (message.type === 'update') {
            ...
            // BUG-FIX: marker が消失 (updateFromMarkdown patch / 外部 echo / 他編集) した場合、
            // 直前に JS 変数に保存した cursor 位置を使って復元
            try {
                var pending = window.__pendingDrawioCursor;
                if (pending && pending.node && editor.contains(pending.node) &&
                    Date.now() - pending.timestamp < 60000 /* 60s 以内 */) {
                    editor.focus();
                    var pRange = document.createRange();
                    var maxOff = (pending.node.nodeType === 3 /* text */) ?
                        pending.node.textContent.length :
                        pending.node.childNodes.length;
                    var safeOff = Math.min(pending.offset, maxOff);
                    pRange.setStart(pending.node, safeOff);
                    pRange.setEnd(pending.node, safeOff);
                    pRange.insertNode(img);
                    pRange.setStartAfter(img);
                    pRange.setEndAfter(img);
            ...
            } catch (ePend) { /* ignore — fall through to default */ }

            editor.focus();
            const sel = window.getSelection();
            // BUG-FIX: marker 不在 + selection が editor 外の時、insertNode は editor 外に挿入される。
            // selection が editor 内かを必ず検査し、外なら editor 末尾に新規 <p> として append。
            const selInEditor = !!(sel && sel.rangeCount && (
                editor.contains(sel.getRangeAt(0).startContainer) ||
                sel.getRangeAt(0).startContainer === editor
            ));
            if (sel && sel.rangeCount && selInEditor) {
                const range = sel.getRangeAt(0);
                range.deleteContents();
                ...
                // 最終 fallback: <p> で wrap して editor 末尾に append (raw <img> は markdown
                // 化で位置が不安定になる可能性があるため)
                const tailP = document.createElement('p');
                tailP.appendChild(img);
                editor.appendChild(tailP);
            }
            // 念のため pending cursor も clear
            window.__pendingDrawioCursor = null;
            syncMarkdown();
            ...
            editor.focus();
            const sel = window.getSelection();
            // Selection が editor 外なら editor 末尾に append (insertImageHtml と同じ防御)
            const fileLinkSelInEditor = !!(sel && sel.rangeCount && (
                editor.contains(sel.getRangeAt(0).startContainer) ||
                sel.getRangeAt(0).startContainer === editor
            ));
            if (sel && sel.rangeCount && fileLinkSelInEditor) {
                const range = sel.getRangeAt(0);
                range.deleteContents();
            ...
            const sel = window.getSelection();
            // Selection が editor 外なら editor 末尾に append
            const linkSelInEditor = !!(sel && sel.rangeCount && (
                editor.contains(sel.getRangeAt(0).startContainer) ||
                sel.getRangeAt(0).startContainer === editor
            ));
            if (sel && sel.rangeCount && linkSelInEditor) {
                const range = sel.getRangeAt(0);
                range.deleteContents();
                range.insertNode(a);
                range.setStartAfter(a);
            ...
            // TASK-04 (sprint 20260806-165116): 外部更新でも目次を更新する。
            // TOC の更新経路は従来「open 時の toc 同梱」と「ユーザー編集の syncContent →
            // onTocUpdate」の 2 つだけで、外部編集（AI CLI）の update はここを素通りして
            // 本文だけ変わり目次が古いまま残っていた。
            ...
            //   matcher: img の src または data-markdown-path の **path-suffix** が dPath と一致するか。
            //   - dPath は extension 側の絶対 path (forward slash 統一)
            //   - 完全 absolute 一致 → 確実 hit
            //   - basename 一致 → fallback (異なる folder で同 basename の場合は両方 bump、許容)
            //   ?v=mtime cache-bust で browser 再 fetch を強制。
            try {
                var dPath = (message.path || '').replace(/\\/g, '/');
                var dMtime = message.mtime;
                if (!dPath || dMtime === undefined) return;
                var lastSlash = dPath.lastIndexOf('/');
                ...
                for (var ii = 0; ii < imgs.length; ii++) {
                    ...
                    if (!src) continue;
                    var srcNoQuery = src.split('?')[0].split('#')[0].replace(/\\/g, '/');
                    var srcDecoded = decodeSafe(srcNoQuery);
                    // 1) 絶対 path full match (vscode-resource URL は dPath を含む)
                    var fullMatch = srcDecoded.indexOf(dPath) !== -1;
                    if (!fullMatch) {
                        // 2) basename fallback (同 basename / 異 folder のケースは両方 bump、許容)
                        var srcLastSlash = srcDecoded.lastIndexOf('/');
                        var srcBasename = (srcLastSlash >= 0 ? srcDecoded.slice(srcLastSlash + 1) : srcDecoded);
                        if (srcBasename !== basename) continue;
                    }
                    // bump ?v=mtime — 確実な再 fetch のため、まず空 src で flush してから再設定
                    // (一部 webview で同 URL の query 違い再 fetch がキャッシュされる事例の対策)
                    var newSrc = srcNoQuery + '?v=' + dMtime;
                    if (img.getAttribute('src') === newSrc) {
                        // 同 mtime の重複通知 → 強制 reload のため一旦空にして set し直す
                ...
                logger.log('[drawioFileChanged] dPath=' + dPath + ' matched ' + matched + ' img(s)');
            } catch (err) {
    ...
    function showDragCursor(x, y, targetEditorEl) {
        ...
        if (range) {
            ...
            if (rect.height > 0) {
                ...
                // TC-MX-09 (再オープン④): element が editor 自体（コンテンツ外の余白）のときは
                // 要素基準を使わない — editor rect 基準だと「エディタ端に editor 全高のバー」が
                // 出て pointer から数百 px 乖離する。マウス位置 + 20px（range=null fallback と同形）にする。
                if (element === editorEl) {
                    dragCursor.style.left = x + 'px';
                    dragCursor.style.top = y + 'px';
                    dragCursor.style.height = '20px';
                    dragCursor.style.display = 'block';
                } else if (element && editorEl.contains(element)) {
                    const elemRect = element.getBoundingClientRect();
    ...
    if (isMainInstance) document.addEventListener('drop', function(e) {
        ...
        // FR-TF-19 (§4m 2026-08-10 再オープン⑤): outliner / 他 md からの D&D 受け 4 分岐。
        // 従来 md editor は tree 系 2 MIME しか受けず、outliner 📎/page node・md 📎/subpage リンクの
        // drop は main md 宛でも素通りだった（マトリクス未実装セル）。dispatch 順は tree 系の後。
        // main/sidepanel は targetHost 解決（findTargetEditor）で対称 — sidepanel は SidePanelHostBridge が
        // 第 2 引数 filePath を積んで委譲する。cross-note の解決は host 側（FR-TF-18 §4l）。
        try {
            var onfRaw = e.dataTransfer ? e.dataTransfer.getData('application/x-fractal-out-node-file') : '';
            if (onfRaw) {
                var onfPayload = null;
                try { onfPayload = JSON.parse(onfRaw); } catch (err) { /* ignore */ }
                if (onfPayload && onfPayload.outFileKey && onfPayload.nodeId &&
                    typeof targetHost.attachOutNodeFileToMd === 'function') {
                    targetHost.attachOutNodeFileToMd(onfPayload);
                }
                return;
            }
            var onpRaw = e.dataTransfer ? e.dataTransfer.getData('application/x-fractal-out-node-page') : '';
            if (onpRaw) {
                var onpPayload = null;
                try { onpPayload = JSON.parse(onpRaw); } catch (err) { /* ignore */ }
                if (onpPayload && onpPayload.outFileKey && onpPayload.pageId &&
                    typeof targetHost.importOutPageNodeToMd === 'function') {
                    targetHost.importOutPageNodeToMd(onpPayload);
                }
                return;
            }
            ...
            if (mflRaw) {
                ...
                if (mflPayload && mflPayload.href && mflPayload.sourceMdPath &&
                    mflPayload.sourceMdPath !== selfMdPath &&
                    typeof targetHost.attachMdFileLinkToMd === 'function') {
                    targetHost.attachMdFileLinkToMd(mflPayload);
                }
                return;
            }
            ...
            if (mspRaw) {
                ...
                if (mspPayload && mspPayload.href && mspPayload.sourceMdPath &&
                    mspPayload.sourceMdPath !== selfMdPath &&
                    typeof targetHost.linkMdSubpageToMd === 'function') {
                    targetHost.linkMdSubpageToMd(mspPayload);
                }
                return;
            }
        } catch (err) { /* ignore */ }
    ...
    // 選択範囲 → markdown/HTML シリアライズ（copy/cut 共通 — sprint 20260728-135515）。
    // 従来 cut は mdProcessNode の単純ループを独自に持ち、「先頭テキスト + ネストリスト」の
    // li ラップ（下記 else 分岐）が効かず `text- child` 連結の壊れた md が clipboard に入っていた。
    function serializeSelectionToMd(range, sel) {
        const fragment = range.cloneContents();
        const tempDiv = document.createElement('div');
        tempDiv.appendChild(fragment);
        ...
        return { md: md, selectedHtml: selectedHtml };
    }
    ...
    editor.addEventListener('copy', function(e) {
        ...
        try {
            ...
            const md = _ser.md;
            const selectedHtml = _ser.selectedHtml;
    ...
    // sprint 20260728-135515: シリアライズを copy と共通化（serializeSelectionToMd）+
    // 削除後の空 li 殻掃除（Backspace 範囲選択と同型）を追加。従来は mdProcessNode 単純ループで
    // 「text- child」連結 md を吐き、deleteContents の空 li 殻（•/○/▪）が残っていた。
    editor.addEventListener('cut', function(e) {
        ...
        e.preventDefault();
        markAsEdited();
        // undo 用に cut 前の状態を必ず積む（paste :saveSnapshot / Backspace :7591 と同格。
        // 従来 cut だけ欠けており、cmd+z が「cut 前」でなく直前スナップショット時点に戻っていた。
        // markdown 変数は rAF 同期で遅延しうるため、DOM から取り直して正確な pre-cut を捕捉する）
        markdown = htmlToMarkdown();
        undoManager.saveSnapshot();
        ...
        try {
            const _ser = serializeSelectionToMd(range, sel);
            e.clipboardData.setData('text/plain', _ser.md);
            e.clipboardData.setData('text/html', _ser.selectedHtml);
            e.clipboardData.setData('text/x-any-md', _ser.md);
        ...
        for (const li of affectedLis) {
            ...
            // 直下に text / img 等の実コンテンツが残っていれば殻ではない。
            // checkbox(input) は「cut でテキストが全部消えた task li」では残しても空タスク殻
            // （Backspace :7699 の empty 規約 = br/checkbox/nested list のみなら空、と同じ）なので
            // content とみなさない。選択に触れていない子 li の checkbox は昇格側（isTrulyEmptyShell）が守る。
            let hasContent = false;
            for (const child of li.childNodes) {
                ...
                if (child.nodeType === 1) {
                    ...
                    if (tag === 'ul' || tag === 'ol' || tag === 'br' || tag === 'input') continue;
                    // img は textContent 空でも content（TASK-03 / asset 1:1 ownership）
                    if (tag === 'img') { hasContent = true; break; }
                    if (child.textContent.trim() || (child.querySelector && child.querySelector('img'))) { hasContent = true; break; }
                }
            }
    ...
    editor.addEventListener('paste', function(e) {
        ...
        // outliner ノード paste (FR-NP-01 / sprint 20260727-124904): text/html の
        // data-outliner-clipboard メタに添付 (page md / ファイル / 画像) を持つ node があれば
        // 添付複製経路へ (host が OutlinerClipboardStore からソース dir を解決し、複製 +
        // subpage/📎 リンク付き md を pasteWithAssetCopyResult で返す = ADRL-0001)。
        // 添付ゼロなら従来経路 (HtmlMdConverter リスト化) に fall-through — 挙動不変。
        if (html && html.indexOf('data-outliner-clipboard=') !== -1 &&
            typeof host.pasteOutlinerNodesWithAssets === 'function') {
            try {
                const metaMatch = html.match(/data-outliner-clipboard="([^"]+)"/);
                if (metaMatch) {
                    const meta = JSON.parse(decodeURIComponent(metaMatch[1]));
                    const nodes = (meta && meta.nodes) || [];
                    const hasAssets = nodes.some(n =>
                        (n.isPage && n.pageId) || n.filePath || (n.images && n.images.length));
                    if (hasAssets) {
                        // caret snapshot (TASK-B4 / review MED-2): 既存 pasteWithAssetCopy 経路
                        // (:17726-17740) と同じ。無いと async 受信が editor.focus() フォール
                        // バックで先頭挿入 + 直前 paste の stale snapshot 再利用リスク。
                        try {
                            const sel1 = window.getSelection();
                            if (sel1 && sel1.rangeCount > 0) {
                                const r1 = sel1.getRangeAt(0).cloneRange();
                                self._pendingPasteSelection = {
                                ...
                                self._pendingPasteSelection = null; // stale snapshot を持ち越さない
                            }
                        } catch (selErr) {
                            logger.warn('outliner-paste selection snapshot failed:', selErr);
                            self._pendingPasteSelection = null;
                        }
                        host.pasteOutlinerNodesWithAssets(text || '', nodes);
                        return; // pasteWithAssetCopyResult を待つ (既存受信が挿入)
                    }
                }
            } catch { /* meta 解析失敗 → 従来経路に fall-through */ }
        }
        ...
        if (internalMd) {
        ...
        } else if (html && typeof HtmlMdConverter !== 'undefined') {
            ...
            try {
                ...
                logger.error('html-md-converter error:', err);
                pastedMd = text || '';
            }
        } else {
            pastedMd = text || '';
    ...
    editor.addEventListener('blur', function() {
        ...
        if (!isSourceMode && hasUserEdited && queuedExternalContent !== null) {
            ...
            var _finfo = {
                instance: isMainInstance ? 'main' : 'side-panel',
                domLen: _fdomMd.length,
                queueLen: queuedExternalContent.length,
                delta: queuedExternalContent.length - _fdomMd.length
            };
            console.warn('[Fractal:blur-with-queue]', _finfo);
        }
        ...
        if (!isSourceMode && hasUserEdited && (pendingSync || isActivelyEditing)) {
            ...
            markdown = htmlToMarkdown();
            host.syncContent(markdown);
            didFlushUserEdit = true;
            // Fix A (sprint v14, hotfix): user の最新 typing が「真」。 cross-edit から
            // 来ていた古い queue を破棄して、後続の applyQueuedExternalChange による
            // DOM 巻き戻し → disk 上書きの連鎖を防ぐ。
            // NT-14 への影響: queue drop は editing editor の webview 内のみ。host 側
            //   の sidePanelManager listener / editorProvider listener は不変なので、
            //   別 editor への cross-edit forward は維持される。drop された queue 内容
            //   は cross-edit の round-trip で次の event 時に再到来する。
            queuedExternalContent = null;
        }
        ...
        // Apply queued external changes only if user did NOT just flush.
        // Flushing implies user typing is the truth; queue would be stale
        // cross-edit content, applying it would cause the rollback bug.
        if (!didFlushUserEdit) {
            applyQueuedExternalChange();
        }
    });
    ...
    sourceEditor.addEventListener('blur', function() {
        ...
        if (isSourceMode && hasUserEdited && queuedExternalContent !== null) {
            var _finfo2 = {
                instance: isMainInstance ? 'main' : 'side-panel',
                domLen: sourceEditor.value.length,
                queueLen: queuedExternalContent.length,
                delta: queuedExternalContent.length - sourceEditor.value.length
            };
        ...
        if (isSourceMode && hasUserEdited && (sourceEditor.value !== markdown || isActivelyEditing)) {
            ...
            // Fix A (sprint v14): editor.blur と同等の queue drop。詳細はそちらのコメント参照
            queuedExternalContent = null;
        }
    ...
    this._handleVisibilityChange = function() {
        ...
        if (document.visibilityState === 'hidden' && hasUserEdited
            ...
            if (queuedExternalContent !== null) {
                ...
                var _finfo3 = {
                    instance: isMainInstance ? 'main' : 'side-panel',
                    domLen: _fdomMd3.length,
                    queueLen: queuedExternalContent.length,
                    delta: queuedExternalContent.length - _fdomMd3.length
                };
                console.warn('[Fractal:blur-with-queue]', _finfo3);
            }
            ...
            host.syncContent(markdown);
            // Fix A (sprint v14): visibility hidden での flush も blur と同様に
            // queue を drop。後続の applyQueuedExternalChange は元々呼んでいないので
            // queue だけ捨てて DOM 巻き戻しの原因経路を断つ。
            queuedExternalContent = null;
        }
    };
