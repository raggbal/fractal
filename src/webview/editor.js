...
/**
 * P2 ミニマム観測ヘルパー (sprint 20260424-135027-debug-banner-outliner-actions)
 *
 * blur 時に queuedExternalContent が non-null + hasUserEdited の状態を検知した時、
 * webview 右上に floating banner を表示してユーザーに通知する。
 * 同時に console.warn '[Fractal:blur-with-queue]' で構造化ログ出力 (call site で実施)。
 *
 * 目的: 「20 分編集 → 別アプリ → 戻ったら巻き戻る」現象の真因特定のための
 *       実機データ採取。修正自体は行わない (NT-14 cross-editor sync 維持)。
 *
 * 将来: 真因確定後 (Sprint v15+) はこのヘルパーと call site を削除予定。
 */
function _fractalDebugShowBanner(info) {
    var existing = document.getElementById('fractal-debug-banner');
    if (existing) existing.remove();
    var banner = document.createElement('div');
    banner.id = 'fractal-debug-banner';
    banner.style.cssText = 'position:fixed;top:10px;right:10px;background:#ff9800;color:#000;padding:10px 14px;border-radius:4px;font-size:12px;font-family:monospace;z-index:99999;max-width:480px;box-shadow:0 2px 8px rgba(0,0,0,0.5);cursor:pointer;line-height:1.5';
    banner.innerHTML = '<strong>⚠️ Fractal: 巻き戻り検知</strong><br>' +
        info.instance + ' / domLen=' + info.domLen + ' / queueLen=' + info.queueLen + ' / delta=' + info.delta + '<br>' +
        '<small>Cmd+Opt+I → Console "[Fractal:blur-with-queue]" を共有 / クリックで閉じる</small>';
...
class EditorInstance {
    ...
    editor.addEventListener('blur', function() {
        ...
        // 実機の「巻き戻り」現象データ採取用、修正は行わない (NT-14 維持)
        if (!isSourceMode && hasUserEdited && queuedExternalContent !== null) {
            ...
            _fractalDebugShowBanner(_finfo);
        }
    ...
    sourceEditor.addEventListener('blur', function() {
        ...
        if (isSourceMode && hasUserEdited && queuedExternalContent !== null) {
            ...
            _fractalDebugShowBanner(_finfo2);
        }
    ...
    this._handleVisibilityChange = function() {
        if (document.visibilityState === 'hidden' && hasUserEdited) {
            ...
            if (queuedExternalContent !== null) {
                ...
                _fractalDebugShowBanner(_finfo3);
            }
