...
(function() {
    ...
    const LUCIDE_ICONS = {
        ...
        'navigateBack': '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>',
        'navigateForward': '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>',
        'attachments': '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l8.57-8.57A4 4 0 1 1 17.93 8.8l-8.58 8.57a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>',
    };
    ...
    function normalizeMultiLineTableCells(text) {
        ...
        // (e.g., Notion exports flatten all rows into one line: `| h1 | <br> | --- | <br> | c1 |`).
        //
        // BUG-FIX: 旧実装は単純に `\|\s*<br>\s*(?=\|)` を全文 replace していたため、
        //   `| <br> | <br> |` のような **正当な空 cell 行** も誤って row 分割してしまい、
        //   table が壊れる症状があった。
        //
        //   修正: line ごとに処理 → 「flattened table」判定の signal は「同 line 内に
        //   `| <br> | --- |` (header→sep の `<br>` 区切り) パターンを含む」こと。
        //   通常の多行 markdown table の `| <br> |` cell は separator を伴わないので影響なし。
        text = text.split('\n').map(function(line) {
            // Heuristic: a flattened Notion table has `| <br> ` immediately followed by
            // a separator-cell signature `| --- |` somewhere on the SAME line.
            if (/\|\s*<br>\s*\|\s*:?-{3,}:?/i.test(line)) {
