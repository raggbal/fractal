...
var MindmapRender = (function() {
    ...
    var ICON_GAP = 6;
    // iteration 31: clone 実測とライブ box レイアウトのサブピクセル/フォントメトリクス差で
    // slack=0 になり実機で折り返すのを防ぐ安全余白。interactions.js A7_SAFETY と同値。
    var SAFETY = 4;
    ...
    function estimateMeasure(node, fontSize) {
        ...
        var iconPad = (node && (node.isPage || node.filePath)) ? (fs * 1.2 + ICON_GAP + SAFETY) : 0;
        var longest = 0;
    ...
    function measureRealWidth(box, node, fontSize) {
        ...
        // iconPad: box 内にアイコン (📄/📎) があると同じ flex 行に icon + gap(6px) が並ぶ分、
        // text が使える幅が減る。iteration 30 は iconPad=PAD_H(20) の固定推定にしていたが、
        // 実機 (Electron) の emoji グリフは headless より広く、icon(実測~15-18px)+gap(6)=~21-24px >
        // 20 で 1〜数 px 不足 → アイコン付き短文が折り返した (ユーザー報告 iteration 31)。
        // → アイコン要素の実 DOM 幅を実測し + gap(6) を足す (推定でなく実測。全 emoji 幅に頑健)。
        // iconPad: アイコン付きノードのみ。icon 実測幅 + gap + SAFETY を足す。
        //   iteration 30 の固定推定 PAD_H(20) では実機 Electron の emoji 幅で不足しアイコン付き短文が
        //   折り返した (iteration 31)。アイコン**無し**ノードは iter30 の BORDER_W だけで OK
        //   (TC-M12 #10「右空白が過大でない」を維持するため SAFETY はアイコン付きだけに限定)。
        ...
        if (iconEl) {
            ...
            iconPad = iconW + ICON_GAP + SAFETY; // アイコン実幅 + flex gap + 安全余白
        }
