...
var MindmapInteractions = (function() {
    ...
    function attach(treeEl, model, settings, host, ctx, runtime) {
        ...
        function handleKeydown(e) {
            ...
            // Alt(option)+↑/↓ → 兄弟の上/下と位置入れ替え（一番上/下なら no-op）。
            // outliner の Cmd+Shift+Arrow と同じ model.moveUp/moveDown を使う（mindmap は Alt が空き）。
            // title 中心ノードは兄弟を持たないので対象外。
            if (e.altKey && !mod && !e.shiftKey && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
                e.preventDefault();
                if (isTitle) { return; }
                pushUndo();
                var moved = (e.key === 'ArrowUp') ? model.moveUp(nodeId) : model.moveDown(nodeId);
                if (moved) {
                    scheduleSync();
                    rerender();
                    focusNode(nodeId, false); // 編集開始せず選択のみ維持（swap 後も同ノードがアクティブ）
        ...
        function adjustEditWidth(nid) {
            ...
            // iteration 31: アイコン付きは icon 実測幅 + gap(6) を足す (推定 A7_PAD_H では
            //   実機 emoji 幅で不足しアイコン付き短文が折り返した)。render.js measureRealWidth と同方式。
        ...
        // ★ HTML5 Drag API（draggable 属性 + dragstart/drop）は SVG 要素（mindmap-node は
        //   foreignObject = SVG 名前空間）では Chromium/Electron が draggable を honor せず native drag が
        //   起動しない（実機で「ドラッグ自体できない」）。→ mouse(pointer) イベントで自前実装する。
        //   mousedown で候補を記録 → 閾値超え mousemove でドラッグ開始 → mouseup で elementFromPoint により
        //   drop 先を解決。frac(上下1/3=兄弟 / 中央=reparent) の判定ロジックは従来と同一。
        //   空白（node 以外）への drop は無効化（キャンセル）— 以前の Floating Topic 化は廃止。
        var DRAG_THRESHOLD = 4; // px。これ未満の移動は click 扱い（drag しない）
        var _mmDown = null;     // { nodeId, x, y, started }

        function clearDropTargets() {
            // 旧 mm-drop-target + 3 状態マーク（above/below/child）を全クリア
            var ts = treeEl.querySelectorAll('.mm-drop-target, .mm-drop-above, .mm-drop-below, .mm-drop-child');
            for (var i = 0; i < ts.length; i++) {
                ts[i].classList.remove('mm-drop-target', 'mm-drop-above', 'mm-drop-below', 'mm-drop-child');
            }
        }
        // clientX/Y 直下の drop 対象 node（title 除く）を返す。無ければ null。
        function dropTargetAt(clientX, clientY, draggedId) {
            ...
            var tid = nodeEl.getAttribute('data-node-id');
            if (!tid || tid === draggedId || model.isDescendant(tid, draggedId)) { return null; }
            return nodeEl;
        }
        // 対象ノード box の縦位置(clientY)から drop ゾーンを返す。マーク表示と drop 判定で共有し
        // 「線が出た位置＝実際の落下先」を保証する（frac 上端1/3=above=兄 / 下端1/3=below=弟 / 中央=child=子）。
        function dropZoneAt(clientY, rect) {
            var frac = (clientY - rect.top) / (rect.height || 1);
            if (frac < 0.33) { return 'above'; }
            if (frac > 0.67) { return 'below'; }
            return 'child';
        }

        on(treeEl, 'mousedown', function(e) {
            if (e.button !== 0) { return; }
            var nodeEl = e.target && e.target.closest ? e.target.closest('.mindmap-node') : null;
            // title 中心ノードは D&D 対象外
            if (!nodeEl || nodeEl.classList.contains('mindmap-title-node')) { return; }
            // 編集中の text 選択（ドラッグ選択）を阻害しないよう、is-editing 中は D&D しない
            var box = nodeEl.querySelector('.mindmap-node-text');
            if (box && box.classList && box.classList.contains('is-editing')) { return; }
            _mmDown = { nodeId: nodeEl.getAttribute('data-node-id'), x: e.clientX, y: e.clientY, started: false };
        });

        on(document, 'mousemove', function(e) {
            if (!_mmDown) { return; }
            if (!_mmDown.started) {
                if (Math.abs(e.clientX - _mmDown.x) < DRAG_THRESHOLD &&
                    Math.abs(e.clientY - _mmDown.y) < DRAG_THRESHOLD) { return; }
            ...
            // 「どこに落ちるか」を明示する。
            clearDropTargets();
            var t = dropTargetAt(e.clientX, e.clientY, _mmDown.nodeId);
            if (t) {
                var tb = t.querySelector('.mindmap-node-box');
                if (tb) {
                    var zone = dropZoneAt(e.clientY, t.getBoundingClientRect());
                    tb.classList.add(zone === 'above' ? 'mm-drop-above' : zone === 'below' ? 'mm-drop-below' : 'mm-drop-child');
                }
            }
        });
        ...
        on(document, 'mouseup', function(e) {
            ...
            var down = _mmDown;
            _mmDown = null;
            if (!down.started) { return; } // 閾値未満 = click（D&D しない。click ハンドラに委ねる）
            document.body.classList.remove('mm-dragging');
            clearDropTargets();
            var draggedId = down.nodeId;
            _dragState = null;
            var targetEl = dropTargetAt(e.clientX, e.clientY, draggedId);
            if (targetEl) {
                // ノード上 drop: 上下 1/3 = 兄弟順序、中央 = 子として reparent
                var targetId = targetEl.getAttribute('data-node-id');
                var zone = dropZoneAt(e.clientY, targetEl.getBoundingClientRect()); // マーク表示と同一判定
                var targetNode = model.getNode(targetId);
                if (!targetNode) { return; }
                pushUndo();
                // attachFromFloating = moveNode + mindmap.x/y クリア + normalize。
                // 通常ノードは x/y が元々 null なので moveNode と同挙動（既存挙動不変）。
                // 既存の Floating Topic（迷子ノード）を node へ drag し戻したときは stale 座標が
                // 消えて構造ノードに正しく復帰する（ユーザーが迷子ノードを回収する経路）。
                // afterId のセマンティクスは moveNode と同一（attachFromFloating が素通し）。
                if (zone === 'above' || zone === 'below') {
                    var parentId = targetNode.parentId;
                    var afterId = (zone === 'below') ? targetId : prevSiblingId(model, targetId);
                    MindmapModel.attachFromFloating(model, draggedId, parentId, afterId);
                } else {
                    if (targetNode.collapsed) { targetNode.collapsed = false; }
                    MindmapModel.attachFromFloating(model, draggedId, targetId, null);
                }
                scheduleSync();
                rerender();
                ensureNodeVisible(draggedId);
            } else {
                // 空白（node 以外）へ drop → 無効化（キャンセル）。元の親/順序のまま。
                // 以前は Floating Topic 化していたが（parentId=null + rootIds 外 + mindmap.x/y）、
                // outliner モードは rootIds 起点でツリーを描くため floating topic が表示されず
                // 「消えた」ように見える（ユーザー報告 2026-07-21）。node は drop 時に moveNode する
        ...
        on(treeEl, 'contextmenu', function(e) {
            ...
            e.preventDefault();
            closeContextMenu();
            // FR-GC-01 (sprint 20260727-080242): group 枠上の右クリックは Group 用メニュー。
            // 従来は nodeEl のみ判定して group 上が「空白扱い」に落ち、選択済みノードから
            // Create Group が出るバグだった。ノードが枠に重なる場合はノード優先 (nodeEl 先勝ち)。
            if (!nodeEl) {
                var groupEl = e.target.closest ? e.target.closest('.mindmap-group') : null;
                if (groupEl) {
                    var ctxGid = groupEl.getAttribute('data-group-id');
                    if (ctxGid) {
                        // メニュー対象を選択状態に (視覚整合)。既存 click 選択 (:1126) と同じく
                        // rerender で is-selected 枠を反映 (menu は document.body 直下なので消えない)
        ...
        // FR-GC-02: Group rename 用ダイアログ。window.prompt は VS Code webview でブロックされる
        // ため、outliner.js:2652 openRenameColumnDialog と同型のカスタム DOM ダイアログを使う
        // (同じ .outliner-add-col-* CSS クラスを流用 = 追加 CSS 不要)。
        // 確定 (Save/Enter): 同値なら no-op、空文字は label 消去 (枠は残る)。キャンセル/Esc: 変更なし。
        function openRenameGroupDialog(groupId, cur) {
            ...
            function commit() {
                var next = nameInput.value; // 空文字 = label 消去を許可 (trim で弾かない)
                dialog.remove();
                if (next === cur) { return; } // 同値は no-op (undo も積まない)
                pushUndo();
                if (typeof MindmapModel !== 'undefined') {
                    MindmapModel.updateGroup(settings, groupId, { label: next });
                }
                scheduleSync(); rerender();
            }
            ...
            if (typeof MindmapRender !== 'undefined' && MindmapRender._trackBodyEl) {
                MindmapRender._trackBodyEl(dialog); // mindmap 破棄時に一緒に消す (menu と同じ管理)
            }
            setTimeout(function() { nameInput.focus(); nameInput.select(); }, 0);

            nameInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') { e.preventDefault(); commit(); }
                else if (e.key === 'Escape') { e.preventDefault(); dialog.remove(); }
                e.stopPropagation(); // mindmap の keydown (Delete 等) に食われない
            });
        }

        // FR-GC-02/03 (sprint 20260727-080242): Group 用コンテキストメニュー。
        // Rename Group… (prompt → updateGroup) と Delete Group (removeGroup) の 2 項目のみ。
        // Create Group / ノード用項目 / Fit to screen は出さない。
        function buildGroupContextMenu(groupId, x, y) {
            ...
            var grp = (settings.groups || []).filter(function(g) { return g.id === groupId; })[0];
            item('Rename Group…', function() {
                // window.prompt は VS Code webview でブロックされるため使わない
                // (outliner.js:2652 openRenameColumnDialog と同じ判断・同 CSS クラスのカスタムダイアログ)。
                openRenameGroupDialog(groupId, (grp && grp.label) || '');
            });
            item('Delete Group', function() {
                pushUndo();
                if (typeof MindmapModel !== 'undefined') {
                    MindmapModel.removeGroup(settings, groupId);
                }
                markGroupSelected(null);
                scheduleSync(); rerender();
            });
