...
var MindmapInteractions = (function() {
    ...
    function attach(treeEl, model, settings, host, ctx, runtime) {
        ...
        //   drop 先を解決。frac(上下1/3=兄弟 / 中央=reparent)・moveNode・detachToFloating の判定ロジックは従来と同一。
        var DRAG_THRESHOLD = 4; // px。これ未満の移動は click 扱い（drag しない）
        var _mmDown = null;     // { nodeId, x, y, started }

        function clearDropTargets() {
            var ts = treeEl.querySelectorAll('.mindmap-node-box.mm-drop-target');
            for (var i = 0; i < ts.length; i++) { ts[i].classList.remove('mm-drop-target'); }
        }
        ...
        on(document, 'mousemove', function(e) {
            ...
            if (t) { var tb = t.querySelector('.mindmap-node-box'); if (tb) { tb.classList.add('mm-drop-target'); } }
        });
        ...
        on(document, 'mouseup', function(e) {
            ...
            if (targetEl) {
                ...
                var trect = treeEl.getBoundingClientRect();
                var x = e.clientX - trect.left;
                var y = e.clientY - trect.top;
                pushUndo();
