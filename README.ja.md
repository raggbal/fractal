...
- 🗂 **Notes** — フォルダ（"note"）単位で情報を管理。ファイル自体はフラットに保存しつつ、表示上は仮想的なフォルダ/ファイルツリーで構造的に整理 — **任意のファイル添付**（PDF、Excel など）も含めて。横断フルテキスト検索は **添付した PDF / Word / Excel / PowerPoint の中身も対象**（ページ / スライド / シート!セルの位置表示 + 参照元 node へのバックリンク付き）。タブ・履歴管理、Daily Notes も。
...
- **ファイル添付** — 任意のファイル（PDF、Excel など）をドロップすると `[📎 filename](path)` リンクを挿入。クリックで OS デフォルトアプリで開く。リンクテキストは他のリンク同様に編集可能で、全選択して copy/cut すると**リンクごと**移動・複製でき、📎 アイコンのドラッグで md 内移動やツリーへの移動も可能
...
- **フルテキスト検索** — note 内のすべてのアウトライン、サブページ、スタンドアロンのマークダウンを横断検索。**添付した PDF / Word / Excel / PowerPoint の中身も対象**（ページ / スライド / シート!セルの位置表示 + 参照元 node へのバックリンク付き）。ストリーミング表示、クリックでジャンプ
