...
- 🗂 **Notes** — フォルダ（"note"）単位で情報を管理。ファイル自体はフラットに保存しつつ、表示上は仮想的なフォルダ/ファイルツリーで構造的に整理 — **任意のファイル添付**（PDF、Excel など）も含めて。横断フルテキスト検索は **添付した PDF / Word / Excel / PowerPoint と HTML / JSON などテキスト系添付の中身も対象**（ページ / スライド / シート!セルの位置表示 + 参照元 node へのバックリンク付き）。タブ・履歴管理、Daily Notes も。さらに **ローカルフォルダをリンク** — ディスク上の任意フォルダを 🔗 folder link として note と並べて登録し、内蔵フォルダビューでブラウズ、md / PDF / HTML はアプリ内で開き、ツリーとの間はドラッグでファイルを行き来（ディスク上の変化は自動反映）。
...
- **ファイル添付** — 任意のファイル（PDF、Excel など）をドロップすると `[📎 filename](path)` リンクを挿入。クリックで開く — HTML / PDF はアプリ内ビューアで、それ以外は OS デフォルトアプリで。リンクテキストは他のリンク同様に編集可能で、全選択して copy/cut すると**リンクごと**移動・複製でき、📎 アイコンのドラッグで md 内移動やツリーへの移動も可能
...
- **ファイル** — 任意のファイル形式をノードに添付。📎 ノードは HTML / PDF をアプリ内ビューアで、それ以外を OS デフォルトアプリで開く
...
- **何でも取り込み** — Finder **や VS Code エクスプローラ**（エクスプローラからは Shift を押しながら）から .md をファイルツリーへ D&D するだけで note に複製・登録。**任意のファイル**（PDF、Excel、zip など）をドロップすれば note 共有の `files/` フォルダに格納 — クリックで開く（HTML / PDF はアプリ内ビューア、それ以外は OS デフォルトアプリ）
...
- **ローカルフォルダをリンク** — ディスク上の任意フォルダを 🔗 **folder link**（`+linkfd`）として登録し、内蔵の**フォルダビュー**でブラウズ: 名前フィルタ・作成 / リネーム / 複製 / 削除（削除は常に OS ゴミ箱）・md はサイドパネル、PDF / HTML は内蔵ビューアで開けます（ファイル行の 📄/📎 アイコンをシングルクリックでも開く）。ディスク上のファイル変化（ビルドや AI の編集など）は**自動で画面に反映**され、ヘッダの 👁 トグルで隠しファイル（dotfile）の表示も切替可能（folder link ごとに記憶。`.git` や `.ssh` など機密フォルダも見える点に注意）。フォルダビュー ⇄ ツリー ⇄ サイドパネル md の間はドラッグで**移動**になり、ファイルの実体は常に 1 箇所だけに存在します（リモートの VS Code server など OS ゴミ箱が使えない環境では、複製確認済みの元ファイルを直接削除）。リンク先フォルダ自体はただの参照 — 同期・クリーン対象にならず、明示的な操作以外で触られることはありません
- **ビューア内検索** — 開いた HTML / PDF の中を Cmd/Ctrl+F（または 🔍）で検索: ハイライト・件数・前/次。横断検索のヒットをクリックすると検索語入りで開いて該当箇所へジャンプ（PDF は該当ページに着地）。HTML の非表示タブはブラウザの検索と同様に対象外
...
### 📄 内蔵ファイルビューア（HTML & PDF）
AI が作るドキュメントは md だけでなく HTML も増えています — Fractal Note はその流れに乗り、HTML も md と同じ感覚でアプリ内で読めるようにしました。ファイルツリー、アウトライナーの 📎 ノード、マークダウンの 📎 リンク — どこから `.html` / `.pdf` をクリックしても Fractal Note の中で開きます:
...
- **安全で速い** — 表示は一瞬。HTML は既定で静的表示（スクリプトは明示的に許可した時だけ実行）、PDF はローカルレンダで滑らかにテキスト選択できます
